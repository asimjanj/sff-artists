/* ==================================================================
   SFF Artist Profile  –  CSV fetch version (no JSONP, no CSP issues)
   ================================================================== */
(function () {
  /* ---- CONFIG ---- */
  ...
  const sanitize = s =>
    s.replace(/<\\/?\\s*(script|style|iframe|object|embed|link)[^>]*?>/gi,'');

  /* ---- tiny CSV parser ---- */
  function csvToRows (csv) {
    return csv.trim().split(/\\r?\\n/).map(line => {
      // naive split is ok because data has no commas
      // if your sheet may contain commas, use a library
    ...
    try {
      ...
      target.innerHTML = list.map(a => `
        <article class="sff-card">
          ...
          <div class="sff-img"><img src="${a.image||'https://via.placeholder.com/400?text=Artist'}"
               alt="${a.name||'Artist'} profile" loading="lazy"></div>
          <p class="sff-bio">${sanitize(a.profile)}</p>
          <div class="sff-links">
            ${LINKS.filter(([k])=>a[k]).map(([k,l]) =>
                 `<a href="${a[k]}" target="_blank" rel="noopener">${l}</a>`).join('')}
          </div>
        </article>`).join('');
    } catch (err) {
